    ...
    function showGoods(data){
        ...
        for(var i=0; i<data.length; i++){
            if (data[i]['gsx$show']['$t']!=0){
                ...
                out += `<p class="cost"><button type="button" onclick="location.href = 'cart.html'">Подробнее</button></p>`; // out += `<p class="cost"><button type="button" onclick="location.href = 'goods${data[i]['gsx$name']['$t']}.html'">Подробнее</button></p>`;               
                out += `</div>`;
                out += `</div>`;
            }
        }
